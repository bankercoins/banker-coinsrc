...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Bankercoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BankercoinTEST2\r");
            Send(hSocket, "WHO #BankercoinTEST2\r");
        } else {
            // randomly join #Bankercoin00-#Bankercoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Bankercoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bankercoin%02d\r", channel_number).c_str());
        }
