Bankercoin-qt: Qt4 GUI for Bankercoin
===============================
...
Alternatively, install Qt Creator and open the `Bankercoin-qt.pro` file.

An executable named `Bankercoin-qt` will be built.
...
A warning for people using the *static binary* version of Bankercoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Bankercoin is linked against libdb5.3.
...
installed causes Bankercoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
